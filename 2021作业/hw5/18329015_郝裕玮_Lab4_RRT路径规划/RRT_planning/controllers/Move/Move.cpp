...
int main() {
	...
	while(robot->step(timeStep)!=-1){//仿真运行一帧
        ...
        b3=length*3*width/2+(width/2+7)*3;//图像中间一行的中间靠右的某像素点
        b4=length*3*width/2+(length-1)*3;//图像中间一行的最右边的某像素点
        //其中b1，b2代表图像中间的黑色轨迹的两侧(大致估计)

		//以rgb的r为标准，当颜色为黑时，r的值必定小于60(10-30左右)
        if(a[b1]<60 && a[b2]>60 && a[b3]>60 && a[b4]>60){//若只有最左边像素点为黑色，则小车需要右转弯使得轨道黑线向中间靠拢
			for(i=0;i<=3;i++){
                speed[i]=speed_rightCircle[i];//速度方向为右转
			}
        }

        else if(a[b1]>60 && a[b2]<60 && a[b3]<60 && a[b4]>60){//若中间两个像素判断点为黑色，则小车可选择继续直行
			for(i=0;i<=3;i++){
		...
		//修改部分2
            ...
            //若左边像素判断点为黑色加上中间两个像素判断点中有一个为黑色，则小车需要右转
			for(i=0;i<=3;i++){
				speed[i]=speed_rightCircle[i];//速度方向为右转
			}
        }
            ...
            //若右边像素判断点为黑色加上中间两个像素判断点中有一个为黑色，则小车需要左转
			for(i=0;i<=3;i++){
				speed[i]=speed_leftCircle[i];//速度方向为左转
			}
        }

        else if(a[b1]>60 && a[b2]>60 && a[b3]>60 && a[b4]>60){//若四个判断像素点全为白色，则小车可选择继续直行
			for(i=0;i<=3;i++){
				//speed[i]=speed_forward[i];//速度方向为直行
				speed[i]=0;
			}
